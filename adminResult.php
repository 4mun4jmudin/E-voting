...
<body>
    ...
    <div class="content">
        ...
        ?>
            <!-- ===========php================================ -->
            <h4 style="text-transform: capitalize;"><b><?php echo $FullName ?> :</b></h4>
            <div class="progress">
        ...
        include("koneksi.php");
        $select = "select MAX(Votes)as MAX from kandidat";
        $run = mysqli_query($conn, $select);
        ...
        ?>
            ...
            <table class="table table-hover" style="overflow-x:auto;">
                ...
                <?php
                $select = "select * from kandidat where Votes='$rVotes'";
                $run = mysqli_query($conn, $select);
                $s = 0;
                while ($row_user = mysqli_fetch_array($run)) {
                    $Nim = $row_user['Nim'];
                ...
                ?>
                    <tbody>
                        <tr style="text-transform: capitalize;">
                            ...
                            <td><a href="adminResult.php?dcl=<?php echo $Nim ?>" class="btn btn-primary" title="Declared"><i class="fa fa-bullhorn"></i></a>
                                <a href="adminResult.php?udcl=<?php echo $Nim ?>" class="btn btn-danger" title="UnDeclared"><i class="fas fa-trash-alt"></i></a>
                            </td>
                            <!-- <td><a href="result.php?show=<?php //echo $PartyName 
                                                                ?>" class="btn btn-primary" title="Declared"><i class="fa fa-bullhorn"></i></a></td> -->
                        </tr>
                <?php }
            } else {
            ...
            if (isset($_GET['dcl'])) {
                ...
                $update = "update kandidat set Status='ON' where FullName='$dcl_PartyName'";
            ...
            if (isset($_GET['udcl'])) {
                ...
                $update = "update kandidat set Status='OFF' where FullName='$udcl_PartyName'";
                $nrun_dcl = mysqli_query($conn, $update);
                if ($nrun_dcl === true) {
            ...
            <!-- =========================Undeclared======================= -->
            <!-- Bar Chart -->
            <h2 style="text-align: center;font-weight: bolder;">Votes Chart :</h2>
            <canvas id="votesChart" width="400" height="200"></canvas>
            <script>
                // Fetch data from the server
                fetch('fetch_votes_data.php')
                    .then(response => response.json())
                    .then(data => {
                        // Extract data for labels and votes
                        const labels = data.map(candidate => candidate.FullName);
